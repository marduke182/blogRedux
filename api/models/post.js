...
import { converter as Converter } from 'showdown-ghost';
import ghostBookshelf from './base';
import config from '../config';
import baseUtils from './base/utils';

const converter = new Converter({extensions: ['ghostgfm', 'footnotes', 'highlight']});


export const Post = ghostBookshelf.Model.extend({

  tableName: 'posts',

  emitChange(event, usePreviousResourceType) {
    let resourceType = this.get('page') ? 'page' : 'post';
    if (usePreviousResourceType) {
      resourceType = this.updated('page') ? 'page' : 'post';
    }
    console.log(resourceType + '.' + event);
  },
  ...
  initialize() {
    ...
    this.on('saved', (model, response, options) => {
  ...
  saving: (model, attr, optionsArg) => {
    let title;

    const options = optionsArg || {};
    // keep tags for 'saved' event and deduplicate upper/lowercase tags
    const tagsToCheck = this.get('tags');
    this.myTags = [];

    _.each(tagsToCheck, (item) => {
      for (let index = 0; index < self.myTags.length; index = index + 1) {
        if (self.myTags[index].name.toLocaleLowerCase() === item.name.toLocaleLowerCase()) {
          return;
        }
      }
  ...
  creating(model, attr, optionsArg) {
    const options = optionsArg || {};
   ...
   */
  updateTags(savedModel, response, optionsArg) {
    const newTags = this.myTags;
    const TagModel = ghostBookshelf.model('Tag');

    const options = optionsArg || {};

    const doTagUpdates = (options) => { // eslint-disable-line
      const currentPostPromise = baseUtils.tagUpdate.fetchCurrentPost(Post, savedModel.id, options);
      const existingTagsPromise = baseUtils.tagUpdate.fetchMatchingTags(TagModel, newTags, options);
      return Promise.all([currentPostPromise, existingTagsPromise])
        .then(([currentPost, existingTagsArg]) => {
          const currentTags = currentPost.related('tags').toJSON(options);
          const existingTags = existingTagsArg ? existingTagsArg.toJSON(options) : [];
          const tagOps = [];
          let tagsToRemove;
          let tagsToCreate;
          ...
          tagsToRemove = _.reject(currentTags, (currentTag) => {
            if (newTags.length === 0) {
              return false;
            }
            return _.any(newTags, (newTag) => {
          ...
          tagsToCreate = _.pluck(_.reject(newTags, (newTag) => {
            return _.any(existingTags, (existingTag) => {
              return baseUtils.tagUpdate.tagsAreEqual(existingTag, newTag);
            });
          }), 'name');

          // Remove any tags which don't exist anymore
          _.each(tagsToRemove, (tag) => {
          ...
          _.each(newTags, (newTag, index) => {
            ...
            if (tagsToCreate.indexOf(newTag.name) > -1) {
              ...
              tag = _.find(currentTags, (currentTag) => {
                return baseUtils.tagUpdate.tagsAreEqual(currentTag, newTag);
              });
              ...
              tag = _.find(existingTags, (existingTag) => {
                return baseUtils.tagUpdate.tagsAreEqual(existingTag, newTag);
              });
    ...
    return ghostBookshelf.transaction((transaction) => {
  ...
  findOne(data, optionsArg) {
    ...
    const prevRelations = _.transform(options.include, (relations, include) => {
    ...
    data = _.defaults(data || {}, { // eslint-disable-line
      status: 'published'
    });
    ...
    return ghostBookshelf.Model
      .findOne.call(this, data, options)
      .then((post) => {
   ...
   */
  destroyByAuthor(optionsArg) {
    // TODO: Must check if this affect the code. Maybe must modify the object after the post is created????
    const postCollection = Posts.forge(); // eslint-disable-line
    const authorId = optionsArg.id;

    const options = this.filterOptions(optionsArg, 'destroyByAuthor');
    if (authorId) {
      return postCollection.query('where', 'author_id', '=', authorId)
        .fetch(options)
        .then((results) => {
          const promises = results.models.map((post) => {
            return post.related('tags')
              .detach(null, options)
              .then(() => {
  ...
  permissible(postModelOrId, action, context, loadedPermissions, hasUserPermission, hasAppPermission) {
    ...
    if (_.isNumber(postModelOrId) || _.isString(postModelOrId)) {
      ...
      return this
        .findOne({id: postModelOrId, status: 'all'})
        .then((foundPostModel) => {
    ...
    if (postModel) {
      ...
      hasUserPermission = hasUserPermission || context.user === postModel.get('author_id'); // eslint-disable-line
    }
...
export const Posts = ghostBookshelf.Collection.extend({
  model: Post
});
