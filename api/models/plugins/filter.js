...
const filterUtils = {
   ...
   * @returns {*}
   */
  combineFilters(enforcedArg, defaultsArg, ...customArg) {
    let enforced = enforcedArg;
    let defaults = defaultsArg;
    let custom = customArg;

    // Ensure everything has been run through the gql parser
    try {
      enforced = enforced ? (_.isString(enforced) ? gql.parse(enforced) : enforced) : null; // eslint-disable-line
      defaults = defaults ? (_.isString(defaults) ? gql.parse(defaults) : defaults) : null; // eslint-disable-line
      custom = _.map(custom, (arg) => { // eslint-disable-line
        return _.isString(arg) ? gql.parse(arg) : arg;
      });
    } catch (error) {
      errors.logAndThrowError(
        new ValidationError(error.message, 'filter'),
    ...
    if (custom && !_.isEmpty(custom.statements) && enforced && !_.isEmpty(enforced.statements)) {
      custom.statements = gql.json.rejectStatements(custom.statements, (customStatement) => {
    ...
    if (defaults && !_.isEmpty(defaults.statements) && custom && !_.isEmpty(custom.statements)) {
      defaults.statements = gql.json.rejectStatements(defaults.statements, (defaultStatement) => {
...
const filter = (Bookshelf) => {
  const Model = Bookshelf.Model.extend({
    ...
    postProcessFilters: function postProcessFilters(options) {
      const joinTables = this._filters.joins;
     ...
     */
    fetchAndCombineFilters: function fetchAndCombineFilters(optionsArg) {
